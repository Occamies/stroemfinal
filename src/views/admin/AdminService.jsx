...
const AdminService = () => {
  ...
  const handleDelete = (ID, title) => {
    ...
    if (window.confirm("Er du sikker på at du vil slette services: " + title)) {
      ...
      makeRequestDelete("http://localhost:5333/service/admin/" + ID, { "Content-Type": "multipart/form-data" }, null, "DELETE")
        ...
        <tbody>
          {
            data && data.map(item =>
              ...
              <tr key={item._id} className="admin-tr">
                <td className="admin-td">{item.title} <img src={`http://localhost:5333/images/service/${item.image}`} alt={item.title} className="admin-img" /></td>
                <td>


                  <Link to={"/admin/AdminServiceEdit/" + item._id} >
